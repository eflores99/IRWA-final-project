from myapp.search.search_engine import cleanText
...
def retrieve_docs(ranked_docs, top, corpus):
    ...
    """
    print("> Start retrieving docs.")
    print("Example of ranked_doc in retrieve_docs func:\n", ranked_docs[0], "\n")
    print("Example of the corpus in retrieve_docs func:\n", corpus['0'], "\n")
    doc_info = []
        ...
        for j in corpus.keys():
            if(d_id == corpus[j]['id']):
                if 'media' in corpus[j]['entities'].keys():
                    ...
                    fav_count = corpus[j]["favorite_count"]
                    rt_count = corpus[j]["retweet_count"]
                    doc_date = corpus[j]["created_at"]
                    url = corpus[j]["entities"]["media"][0]['url']
                    hashtags = corpus[j]["entities"]['hashtags']
                    doc_info.append(Document(id, title, description, doc_date, fav_count, rt_count, url, hashtags))
                    # self, id, title, description, doc_date, likes, retweets, url, hashtags
                    #doc_info.append(Document(title, description, doc_date, url))
                else:
                    ...
                    doc_date = corpus[j]["created_at"]
                    hashtags = corpus[j]["entities"]['hashtags']
                    doc_info.append(Document(id, title, description, doc_date, fav_count, rt_count, '', hashtags))       
    print("> Finished retrieving docs.")
    print("Retrieved doc example: \n", doc_info[0], "\n")
    return doc_info